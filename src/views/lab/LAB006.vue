      ...
      <q-form @submit="validarDatos">
        <div class="text-center">
          ...
          <p :class="reg.opcion_lab006 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="reg.opcion_lab006 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="reg.opcion_lab006">
              {{ reg.opcion_lab006 }}
            </q-chip>
          </p>
        </div>
        ...
        <div class="border-format q-my-sm">
          ...
          <p class="row text-justify">
            Procedimiento para examinar la vagina, el útero, las trompas de Falopio, los ovarios y la vejiga. Se introduce un instrumento en la vagina
            que produce ondas de sonido que rebotan en los órganos del interior de la pelvis. Estas ondas de sonido producen ecos que se envían a una
            computadora que crea una imagen llamada ecografía. También se llama ecografía endovaginal y ETV.
          </p>
        </div>
        <div class="border-format q-my-sm">
          <div class="text-center text-subtitle1 text-bold q-py-xs">BENEFICIOS</div>
          <p class="row text-justify">
            La ecografía transvaginal actua como complemento al examen físico, permite la evaluación de útero, trompas, ovarios o masas pélvicas. Dada
            su disponibilidad, comodidad, costo e información que arroja, es de uso rutinario en la consulta ginecológica. Es el examen indicado para
            el diagnóstico y seguimiento de quistes en los ovarios, miomas uterinos, valoración del espesor endometrial y detección temprana de cancer
            de útero y ovarios. En caso de sangrado genital (hemorragia uterina anormal), permite determinar causas y orientar el manejo. Permite la
            evaluación de los órganos reproductivos en cuanto a su estructura y funcionamiento. Asimismo su uso es frecuente para seguir tratamientos
            y evaluar el embarazo en etapas tempranas
          </p>
        </div>
        <div class="border-format q-my-sm">
          <div class="text-center text-subtitle1 text-bold q-py-xs">RIESGOS</div>
          <p class="row text-justify">
            No hay efectos dañinos conocidos de la ecografía trasvaginal en humanos. A diferencia de los rayos X tradicionales, no se presenta
            exposición a la radiación con este examen.
          </p>
        </div>
        <div class="border-format q-my-sm" v-if="reg.opcion_lab006 == 'AUTORIZAR'">
          ...
          <p>
            ...
            <InputF_ v-model="getPaci.cod" />, después de haber sido informado (a) sobre el procedimiento de Ecografía Transvaginal, los riesgos y
        ...
        <div class="border-format q-my-sm" v-if="reg.opcion_lab006 == 'REVOCAR'">
          ...
          <p>
            Yo <InputF_ v-model="getPaci.descrip" width="300" />, identificada (o) con el documento de identidad número
            <InputF_ v-model="getPaci.cod" />, después de haber sido informado (a) sobre el procedimiento de Ecografía Tranvaginal sus riesgos y
...
const imprimirConsen = async (llave) => {
  ...
  try {
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-LAB-006` });
