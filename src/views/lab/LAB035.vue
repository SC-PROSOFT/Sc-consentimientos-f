<template>
  <q-card class="q-mx-auto format">
    <q-form @submit="validarDatos">
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="LAB035.opcion_lab035 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="LAB035.opcion_lab035 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="LAB035.opcion_lab035">
              {{ LAB035.opcion_lab035 }}
            </q-chip>
          </p>
        </div>
        <p>
          <span class="text-bold">Fecha:&nbsp;</span>
          <span>{{ LAB035.fecha }}</span>
        </p>
        <div class="row justify-between items-center">
          <div class="row">
            <p class="text-bold">Remitente:</p>
            <Input_ class="q-ma-xs" v-model="LAB035.remitente" :field="form.remitente" :inputStyle="{ width: '385px' }" />
          </div>
        ...
        <div class="q-mt-lg">
          ...
          <ol type="a">
            <li>
              <q-radio class="q-ml-xs" color="primary" keep-color label="ABS" v-model="LAB035.result_prb" val="A" />
            </li>
            <li>
              <q-radio class="q-ml-xs" color="primary" keep-color label="PC" v-model="LAB035.result_prb" val="B" />
            </li>
            <li>
              <q-radio class="q-ml-xs" color="primary" keep-color label="Otro" v-model="LAB035.result_prb" val="C" />
            </li>
          </ol>
          <div v-if="LAB035.result_prb == 'C'" class="row">
            <p>Cual:</p>
            <Input_ class="q-ma-xs" v-model="LAB035.otr_result" :field="form.otr_result" :inputStyle="{ width: '385px' }" />
          </div>
        </div>
        <div class="q-mt-lg">
          <div class="col-12">
          ...
          <ol type="a">
            <li>
              <q-radio class="q-ml-xs" color="primary" keep-color label="Conducta sexual de riesgo" v-model="LAB035.motivo_prb" val="A" />
            </li>
            <li>
              <q-radio class="q-ml-xs" color="primary" keep-color label="DX perinatal" v-model="LAB035.motivo_prb" val="B" />
            </li>
            <li>
              <q-radio class="q-ml-xs" color="primary" keep-color label="Sospecha clinica" v-model="LAB035.motivo_prb" val="C" />
            </li>
            <li>
              <q-radio class="q-ml-xs" color="primary" keep-color label="Contacto VIH" v-model="LAB035.motivo_prb" val="D" />
            </li>
            <li>
              <q-radio class="q-ml-xs" color="primary" keep-color label="Transfusiones antes de 1993" v-model="LAB035.motivo_prb" val="E" />
            </li>
            <li>
              <q-radio class="q-ml-xs" color="primary" keep-color label="Uso de agujas" v-model="LAB035.motivo_prb" val="F" />
            </li>
            <li>
              <q-radio class="q-ml-xs" color="primary" keep-color label="Tiempo de gestación" v-model="LAB035.motivo_prb" val="G" />
            </li>
          </ol>
        </div>
        <div>
    ...
    <div class="row justify-center q-my-lg">
      <q-btn
        :disable="LAB035.opcion_lab035 ? false : true"
        @click="validarDatos"
...
import { impresionLAB035, impresion, generarArchivo } from "@/impresiones";
...
const datosInit = () => {
  LAB035.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
};
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(LAB035));
  let llave_paci;
  ...
  let datos = {
    ...
    estado: LAB035.opcion_lab035 == "AUTORIZAR" ? "1" : "2",
  ...
  await getDll$({ modulo: `save_consen.dll`, data: { ...datos } })
    .then((data) => {
      if (data?.llave_consen) {
        ...
        LAB035.fecha = dayjs(fecha).format("YYYY-MM-DD");
...
const imprimirConsen = async (llave) => {
  const datos_lab035 = {
    autorizo: LAB035.opcion_lab035 == "AUTORIZAR" ? true : false,
    empresa: getEmpresa,
    testigo: getTestigo,
    paciente: getPaci,
    prof: getProf,
  ...
  const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-LAB-035` });
