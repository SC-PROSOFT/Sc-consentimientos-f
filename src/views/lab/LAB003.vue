      ...
      <q-form @submit="validarDatos">
        <div class="text-center">
          ...
          <p :class="reg.opcion_lab003 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="reg.opcion_lab003 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="reg.opcion_lab003">
              {{ reg.opcion_lab003 }}
            </q-chip>
          </p>
        </div>
        ...
        <div class="border-format q-my-sm">
          ...
          <p class="row text-justify">
            El examen de rayos X ayuda a los médicos a diagnosticar y tratar las condiciones médicas. Lo expone a una pequeña dosis de radiación
            ionizante para producir imágenes del interior del cuerpo. Los rayos X son la forma más antigua y la que se usa con más frecuencia para
            producir imágenes médicas. Una radiografía es una imagen de las estructuras internas del cuerpo producidos por la exposición a una fuente
            controlada de los rayos X y que se registra en un sistema digital como un CD o disco duro que después puede verse en un computador.
          </p>
        </div>
        <div class="border-format q-my-sm">
          <div class="text-center text-subtitle1 text-bold q-py-xs">BENEFICIOS</div>
          <p class="row text-justify">
            Es un examen económico, accesible, rápido, brinda importante información anatómica (para estructuras óseas), por el tipo de aparato que
            emplea se pueden utilizar en personas de las más diversas contexturas físicas. Es por eso que son especialmente útiles en la detección de
            enfermedades del esqueleto, aunque también se utilizan para diagnosticar enfermedades de los tejidos blandos, como neumonía, cáncer de
            pulmón, edema pulmonar, abscesos, entre otros.
          </p>
        </div>
        <div class="border-format q-my-sm">
          <div class="text-center text-subtitle1 text-bold q-py-xs">RIESGOS</div>
          <p class="row text-justify">
            Existen riesgos asociados con los rayos X, pero una radiografía simple utiliza una pequeña cantidad de radiación equivalente a la que
            todos recibimos de la atmósfera durante un período de dos o tres días. Las pacientes que estén o puedan estar embarazadas deben
            informarlo, ya que se debe procurar otro examen diagnóstico que reemplace los rayos X. Si es necesario realizar esta prueba se cubrirá el
            abdomen o la pelvis con un delantal de plomo ya que el feto es más sensible a la radiación.
          </p>
        </div>
        <div class="border-format q-my-sm" v-if="reg.opcion_lab003 == 'AUTORIZAR'">
          ...
          <p>
            ...
            <InputF_ v-model="getPaci.cod" />, después de haber sido informado (a) sobre el procedimiento de Radiografía Convencional, los riesgos y
            beneficios, declaro que la información ha sido clara, que se me han respondido las inquietudes y que autorizo la toma del procedimiento
        ...
        <div class="border-format q-my-sm" v-if="reg.opcion_lab003 == 'REVOCAR'">
          ...
          <p>
            Yo <InputF_ v-model="getPaci.descrip" width="300" />, identificada (o) con el documento de identidad número
            <InputF_ v-model="getPaci.cod" />, después de haber sido informado (a) sobre el procedimiento de Radiografía Convencional sus riesgos y
            beneficios y adicionalmente, los riesgos de no realizármelo, declaro que la información ha sido clara, que se me han respondido las
...
const { getDll$, _getFirma$, _getHuella$, logOut$, guardarFile$, enviarCorreo$, getEncabezado, guardarArchivo$ } = useApiContabilidad();
...
const imprimirConsen = async (llave) => {
  try {
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-LAB-003` });
