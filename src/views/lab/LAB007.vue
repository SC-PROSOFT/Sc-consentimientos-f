      ...
      <q-form @submit="validarDatos">
        <div class="text-center">
          ...
          <p :class="reg.opcion_lab007 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="reg.opcion_lab007 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="reg.opcion_lab007">
              {{ reg.opcion_lab007 }}
            </q-chip>
          </p>
        </div>
        ...
        <div class="border-format q-my-sm">
          ...
          <p class="row text-justify">
            El examen de densidad ósea, también llamada absorciometría de rayos X de energía dual, DEXA o DXA, utiliza una dosis muy pequeña de
            radiación ionizante para producir imágenes del interior del cuerpo, generalmente la parte inferior de la columna (lumbar) y las caderas,
            para medir la pérdida de hueso. Generalmente, se utiliza para diagnosticar osteoporosis, para evaluar el riesgo que tiene un individuo de
            desarrollar fracturas debidas a la osteoporosis. La DXA es simple, rápida, y no es invasiva. También es el método más comunmente utilizado
            y estándar para diagnosticar la osteoporosis
          </p>
        </div>
        <div class="border-format q-my-sm">
          <div class="text-center text-subtitle1 text-bold q-py-xs">BENEFICIOS</div>
          <p class="row text-justify">
            La densitometría ósea de DXA es un procedimiento simple, rápido y no invasivo. No se requiere anestesia. El examen de densidad ósea DXA
            es, actualmente, el mejor método estandarizado disponible para diagnosticar la osteoporosis y también se lo considera un exacto estimador
            del riesgo de fractura. La DXA se utiliza para tomar decisiones con respecto a si se necesita un tratamiento, y puede ser utilizada para
            monitorear los efectos del tratamiento. Los equipos DXA se encuentran ampliamente disponibles haciendo que la densitometría ósea sea
            conveniente tanto para los pacientes y como para los médicos. Luego del examen no queda radiación en su cuerpo. Los rayos X por lo general
            no tienen efectos secundarios en el rango de diagnóstico típico para este examen.
          </p>
        </div>
        <div class="border-format q-my-sm">
          <div class="text-center text-subtitle1 text-bold q-py-xs">RIESGOS</div>
          <p class="row text-justify">
            No se esperan complicaciones en el procedimiento. La cantidad de radiación utilizada es extremadamente pequeña, menos de un décimo de la
            dosis estándar de rayos X para tórax y menos que la exposición de un día a la radiación natural.
          </p>
        </div>
        <div class="border-format q-my-sm" v-if="reg.opcion_lab007 == 'AUTORIZAR'">
          ...
          <p>
            ...
            <InputF_ v-model="getPaci.cod" />, después de haber sido informado (a) sobre el procedimiento de Densitometría Ósea, los riesgos y
        ...
        <div class="border-format q-my-sm" v-if="reg.opcion_lab007 == 'REVOCAR'">
          ...
          <p>
            Yo <InputF_ v-model="getPaci.descrip" width="300" />, identificada (o) con el documento de identidad número
            <InputF_ v-model="getPaci.cod" />, después de haber sido informado (a) sobre el procedimiento de Densitometría Ósea sus riesgos y
