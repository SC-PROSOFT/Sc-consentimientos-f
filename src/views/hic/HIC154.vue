<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic154 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic154 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic154">
              {{ opcion_hic154 }}
            </q-chip>
          </p>
        </div>
        <div class="row q-mt-md q-mb-md">
          <div>
            ...
            <p class="text-justify q-pa-xs">
              La submaxilectomía es la extirpación total de la glándula submaxilar, situada bajo la rama mandibular. La cirugía suele estar indicada
              en caso de inflamación crónica de la glándula por cálculos en su interior o por dificultad en el drenaje de la saliva o bien, porque
              dentro de la misma se desarrolle un tumor de características benignas o malignas que requiera estudio anatomopatológico.
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">EN QUE CONSISTE LA SUBMAXILECTOMIA</p>
            <p class="text-justify q-pa-xs">
              La incisión se realiza habitualmente de forma paralela a la rama mandibular, encima de la glándula submaxilar, de una longitud
              suficiente como para llevar a cabo la extirpación de la misma y utilizando un pliegue del cuello para disimular la cicatriz. La
              disección glandular se hace hasta encontrar el conducto de drenaje de la saliva, que se dirige desde la glándula hacia el interior de la
              boca. debajo de la lengua (conducto de Wharton).
            </p>
            <p class="text-justify q-pa-xs">
              En caso de confirmación de tumor maligno, se asocia un vaciamiento de los ganglios de la zona que también se envían para el análisis
              anatomopatológico. Lo incisión que se precisa suele ser de mayor tamaño cuando es necesario el vaciamiento ganglionar.
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">RIESGOS DE LA SUBMAXILECTOMIA</p>
            <p class="text-justify q-pa-xs">
            ...
            <p class="text-justify q-pa-xs">
              ...
              Hemorragia local que precise reintervención; hematoma postoperatorio, que se reabsorbe sin necesidad de revisión quirúrgica; dolor en la
              zona de la intervención que calma con analgesia: asimetría bucal al hablar o sonreír por lesión del nervio mentoniano (ramo del nervio
              facial), que pasa por la región en donde asienta la glándula (esta asimetría suele ser transitoria y se recupera lentamente en los meses
              siguientes a la intervención); la extirpación de la glándula puede conllevar una ligera depresión submandibular, dependiendo del tamaño
              que previamente tuviera la glándula, que irá desapareciendo con el tiempo.
            </p>
            <p class="text-justify q-pa-xs">
              <span style="text-decoration: underline">Riesgos secundarios:</span>
              Puede presentar una zona de anestesia (acorchamiento) en la región intervenida, que cederá con el tiempo; cicatrización hipertrófica de
              la incisión realizada.
            </p>
            <p class="text-justify q-pa-xs">
              <span style="text-decoration: underline">Complicaciones graves excepcionales: </span>
              Excepcionalmente, el nervio responsable de la motilidad de la lengua puede traumatizarse en la cirugía con parálisis secundaria
          ...
          <div class="col-12">
            ...
            <TextArea_ v-model="HIC154.riesgo_personaliz" :field="form.riesgo_personaliz" class="col-12" />
          </div>
          <div>
        ...
        <div v-if="opcion_hic154 == 'AUTORIZAR'">
          ...
          <div>
            <p class="text-justify q-pa-xs">
              ...
              <span class="text-bold">SUBMAXILECTOMIA.</span>
            </p>
            <p class="text-justify q-pa-xs">
            ...
            <Input_ v-model="HIC154.med_explica" :field="form.med_explica" :inputStyle="{ width: '700px' }" />
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic154 == 'REVOCAR'">
        ...
        <span class="text-bold">Villavicencio, {{ HIC154.fecha }}. </span>
      </q-card-section>
    </div>
    <q-separator />
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic154 ? false : true"
        @click="validarDatos"
...
import { impresionHIC154, impresion, generarArchivo } from "@/impresiones";
...
const HIC154 = reactive({
  fecha: "",
  med_explica: "",
  riesgo_personaliz: "",
});

const opcion_hic154 = ref(null);
...
onMounted(() => {
  HIC154.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC154));
  let datos = {
    nit_entid: nit_usu.value,
    estado: opcion_hic154.value == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic154 = {
      autorizo: opcion_hic154.value == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      testigo: getTestigo,
      paciente: getPaci,
      prof: getProf,
    ...
    const docDefinitionPrint = await utilsFormat({
      datos: { ...firmas, cod_consen: "HIC154" },
      content: impresionHIC154({
        datos: datos_hic154,
      }),
    });
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-154` });
