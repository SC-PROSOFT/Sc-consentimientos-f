<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic160 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic160 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic160">
              {{ opcion_hic160 }}
            </q-chip>
          </p>
        </div>
        <div class="row q-mt-md q-mb-md">
          <div>
            ...
            <p class="text-justify q-pa-xs">
              La obstrucción crónica de las vías lagrimales es un impedimento al drenaje normal que sufre la lágrima, viéndose ésta obligada a gotear
              y caer por el párpado. También, el saco lagrimal así obstruido puede sufrir infecciones crónicas que en ocasiones pueden llevar a la
              formación de un absceso con aparición de una fístula a través de la piel que cubre el saco lagrimal.
            </p>
            <p class="text-justify q-pa-xs">El tratamiento definitivo consiste en realizar una Dacriocistorrinostomía.</p>
            <p class="text-justify q-pa-xs">
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">
              EN QUE CONSISTE LA DACRIOCISTORRINOSTOMIA ENDONASAL
            </p>
            <p class="text-justify q-pa-xs">
              Esta intervención consiste en abrir un agujero desde la cavidad nasal para comunicar con el saco lagrimal obstruido y así permitir el
              paso de la lágrima.
            </p>
            <p class="text-justify q-pa-xs">Esta variante evita la cicatriz de la piel.</p>
            <p class="text-justify q-pa-xs">
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">RIESGOS DE DACRIOCISTORRINOSTOMIA ENDONASAL</p>
            <p class="text-justify q-pa-xs">
              A pesar de la adecuada elección de la técnica y de su correcta realización, pueden presentarse efectos indeseables, tonto los comunes
            ...
            <ul class="text-justify">
              <li>
                <p class="text-justify q-pa-xs">Persistencia del lagrimeo, aproximadamente en un 5%.</p>
              </li>
              <li>
                <p class="text-justify q-pa-xs">Hemorragias severas.</p>
              </li>
              <li>
                <p class="text-justify q-pa-xs">Infecciones graves.</p>
              </li>
              <li>
                <p class="text-justify q-pa-xs">Formación de una cicatriz antiestética.</p>
              </li>
            </ul>
            <p class="text-justify q-pa-xs">Estas tres últimas complicaciones se presentan en un porcentaje muy bajo.</p>
            <p class="text-justify q-pa-xs">
          ...
          <div class="col-12">
            ...
            <TextArea_ v-model="HIC160.riesgo_personaliz" :field="form.riesgo_personaliz" class="col-12" />
          </div>
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">QUE OTRAS ALTERNATIVAS HAY</p>
            <p class="text-justify q-pa-xs">
              Otras formas paliativas de tratamiento abarcan la extirpación del saco lagrimal que solucionaría el problema de las infecciones crónicas
              pero no el del continuo lagrimeo, el masaje diario en el lado de la nariz donde se encuentra el saco lagrimal para evitar la infección y
              formación de un absceso, así como tratamiento con antibióticos sistémicos cada vez que se infecte el saco.
            </p>
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic160 == 'AUTORIZAR'">
          ...
          <div class="row">
            <p class="text-justify">
              ...
              <span class="text-bold">DACRIOCISTORRINOSTOMIA ENDONASAL.</span>
            ...
            <Input_ v-model="HIC160.med_explica" :field="form.med_explica" :inputStyle="{ width: '700px' }" />
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic160 == 'REVOCAR'">
        ...
        <span class="text-bold">Villavicencio, {{ HIC160.fecha }}. </span>
      </q-card-section>
    </div>
    <q-separator />
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic160 ? false : true"
        @click="validarDatos"
...
import { impresionHIC160, impresion, generarArchivo } from "@/impresiones";
...
const HIC160 = reactive({
  fecha: "",
  med_explica: "",
  riesgo_personaliz: "",
});

const opcion_hic160 = ref(null);
...
onMounted(() => {
  HIC160.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC160));
  let datos = {
    nit_entid: nit_usu.value,
    estado: opcion_hic160.value == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic160 = {
      autorizo: opcion_hic160.value == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      testigo: getTestigo,
      paciente: getPaci,
      prof: getProf,
    ...
    const docDefinitionPrint = await utilsFormat({
      datos: { ...firmas, cod_consen: "HIC160" },
      content: impresionHIC160({
        datos: datos_hic160,
      }),
    });
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-160` });
