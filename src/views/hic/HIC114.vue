    ...
    <q-card class="q-mx-auto format q-mb-lg" style="overflow: auto">
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="HIC114.opcion_hic114 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="HIC114.opcion_hic114 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="HIC114.opcion_hic114">
              {{ HIC114.opcion_hic114 }}
            </q-chip>
          </p>
        </div>
      ...
      <div class="row justify-center q-my-lg">
        <q-btn
          :disable="HIC114.opcion_hic114 ? false : true"
          @click="validarDatos"
...
import { impresionHIC114, impresion, generarArchivo } from "@/impresiones";
...
const { getPaci, getAcomp, getProf, getEmpresa, getSesion, getHc, getTestigo } = useModuleFormatos();
...
const datosInit = async () => {
  if (getSesion.novedad == "4") {
    ...
    Object.assign(HIC114, res_consen.value.reg_coninf.datos);
    ...
    HIC114.opcion_hic114 = "AUTORIZAR";
    setTimeout(() => {
  ...
  HIC114.fecha = dayjs(getEmpresa.FECHA_ACT).format("YYYY-MM-DD");
  HIC114.edad = calcularEdad(getAcomp.nacim);
};
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC114));
  console.log("pare da paren_acomp -->  ", getPaci.paren_acomp);

  let datos = {
    nit_entid: nit_usu.value,
    estado: HIC114.opcion_hic114 == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic114 = {
      autorizo: HIC114.opcion_hic114 == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      paciente: getPaci,
      prof: getProf,
      acomp: getAcomp,
...
const agregarServicio = () => {
  ...
  if (editingIndice.value) {
    ...
    // preparar siguiente índice disponible
...
const editarServicio = (row) => {
  ...
  CON851P(
    ...
    () => {
      ...
      reg_tabla_servicio.value.indice_i = row.indice_i; // mantener posición
      ...
      cantidad_items.value = 1; // edición siempre 1
      foco_(form_tabla_servicio, "tipo_serv");
    }
  );
};
