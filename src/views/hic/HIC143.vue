<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic143 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic143 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic143">
              {{ opcion_hic143 }}
            </q-chip>
          </p>
        </div>
        <div class="row q-mt-md q-mb-md">
          <div>
            ...
            <p class="text-justify q-pa-xs">
              Mediante este procedimiento se pretende la extirpación de su lesión con posibilidad de realizar un estudio anatomopatológico de la
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">
              EN QUE CONSISTE LA EXTIRPACIÓN DE LESIÓN BAJO ANESTESIA LOCAL
            </p>
            <p class="text-justify q-pa-xs">
              Tras la infiltración con anestesia local en la zona, se extirpa la lesión (total o parcial). También cabe la posibilidad de que durante
              la cirugía haya que realizar modificaciones del procedimiento por los hallazgos intraoperatorios para proporcionar un tratamiento más
              adecuado.
            </p>
          </div>
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">
              RIESGOS DE LA EXTIRPACIÓN DE LESIÓN BAJO ANESTESIA LOCAL
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div class="col-12">
            ...
            <TextArea_ v-model="HIC143.riesgo_personaliz" :field="form.riesgo_personaliz" class="col-12" />
          </div>
          <div>
        ...
        <div v-if="opcion_hic143 == 'AUTORIZAR'">
          ...
          <div>
            ...
            <Input_ v-model="HIC143.med_explica" :field="form.med_explica" :inputStyle="{ width: '700px' }" />
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic143 == 'REVOCAR'">
        ...
        <span class="text-bold">Villavicencio, {{ HIC143.fecha }}. </span>
      </q-card-section>
    </div>
    <q-separator />
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic143 ? false : true"
        @click="validarDatos"
...
import { impresionHIC143, impresion, generarArchivo } from "@/impresiones";
...
const HIC143 = reactive({
  fecha: "",
  med_explica: "",
  riesgo_personaliz: "",
});

const opcion_hic143 = ref(null);
...
onMounted(() => {
  HIC143.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC143));
  let datos = {
    nit_entid: nit_usu.value,
    estado: opcion_hic143.value == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic143 = {
      autorizo: opcion_hic143.value == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      testigo: getTestigo,
      paciente: getPaci,
      prof: getProf,
    ...
    const docDefinitionPrint = await utilsFormat({
      datos: { ...firmas, cod_consen: "HIC143" },
      content: impresionHIC143({
        datos: datos_hic143,
      }),
    });
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-143` });
