<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic151 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic151 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic151">
              {{ opcion_hic151 }}
            </q-chip>
          </p>
        </div>
        <div class="row q-mt-md q-mb-md">
          <div>
            ...
            <p class="text-justify q-pa-xs">
              Esta técnica se utiliza para explorar directamente la laringe por vía endoscópica con la ayuda de un tubo rígido (laringoscopio) y de un
              microscopio quirúrgico o de una óptica.
            </p>
            <p class="text-justify q-pa-xs">La laringoscopia directa por suspensión permite:</p>
            <ul class="text-justify">
              <li>
                <p class="text-justify q-pa-xs">Precisar la extensión de una lesión en la laringe.</p>
              </li>
              <li>
                <p class="text-justify q-pa-xs">
                  Realizar una cirugía sobre las cuerdas vocales, como en el caso de los nódulos o pólipos laríngeos, o una biopsia de una lesión que
                  requiera estudio anatomopatológico. Habitualmente la técnica es bien tolerada y de corta duración. Es muy importante hacer reposo
                  vocal, es decir, hablar lo menos posible en un periodo de 5-7 días para permitir la cicatrización adecuada de la zona operada. El
                  tipo de anestesia requerida será la indicada por el anestesiólogo. Es posible que, durante o después de la intervención, sea
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">
              EN QUE CONSISTE CIRUGÍA ENDOLARINGEA POR SUSPENSIÓN
            </p>
            <p class="text-justify q-pa-xs">
              La intervención precisa de una suspensión de la laringe mediante un laringoscopio rígido que se introduce por la boca. Los instrumentos
              utilizados a través del tubo y bajo control microscópico o endoscópico permiten la cirugía sobre las cuerdas vocales o la zona a
              explorar.
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">RIESGOS DE LA CIRUGÍA ENDOLARINGEA POR SUSPENSIÓN</p>
            <p class="text-justify q-pa-xs">
            ...
            <p class="text-justify q-pa-xs">
              ...
              Dependiendo de la anatomía particular del cuello, a veces, la exposición de la laringe es insuficiente. Debido a la postura en la
              intervención, puede presentar molestias a nivel del cuello en los días posteriores. Debido a la introducción del tubo, puede presentar
              heridas de pequeña entidad en la boca, lengua o incluso, o nivel dentario, con movilización de alguna pieza que precise extracción.
              También puede presentar molestias en la deglución que desaparecen en unos días. Puede notar sensación de falta de aire por inflamación
              de la laringe tras la cirugía o durante la misma, requiriendo en casos excepcionales intubación traqueal e incluso traqueotomía.
            </p>
            <p class="text-justify q-pa-xs">
              <span style="text-decoration: underline">Riesgos secundarios:</span>
              La voz puede cambiar y hacerse más grave por la presencia de un granuloma o de lesiones cicatriciales o incluso, volver a presentar la
              lesión inicial.
            </p>
            <p class="text-justify q-pa-xs">
              <span style="text-decoration: underline">Complicaciones graves:</span>
              Existe un riesgo muy infrecuente de neumotorax o de hemorragia importante.
            </p>
            <p class="text-justify q-pa-xs">
              Estas complicaciones habitualmente se resuelven con tratamiento médico (medicamentos, sueros, etc.) pero pueden llegara requerir una
          ...
          <div class="col-12">
            ...
            <TextArea_ v-model="HIC151.riesgo_personaliz" :field="form.riesgo_personaliz" class="col-12" />
          </div>
          <div>
        ...
        <div v-if="opcion_hic151 == 'AUTORIZAR'">
          ...
          <div class="row">
            <p class="text-justify">
              ...
              <span class="text-bold">CIRUGÍA ENDOLARINGEA POR SUSPENSIÓN.</span>
            ...
            <Input_ v-model="HIC151.med_explica" :field="form.med_explica" :inputStyle="{ width: '700px' }" />
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic151 == 'REVOCAR'">
        ...
        <span class="text-bold">Villavicencio, {{ HIC151.fecha }}. </span>
      </q-card-section>
    </div>
    <q-separator />
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic151 ? false : true"
        @click="validarDatos"
...
import { impresionHIC151, impresion, generarArchivo } from "@/impresiones";
...
const HIC151 = reactive({
  fecha: "",
  med_explica: "",
  riesgo_personaliz: "",
});

const opcion_hic151 = ref(null);
...
onMounted(() => {
  HIC151.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC151));
  let datos = {
    nit_entid: nit_usu.value,
    estado: opcion_hic151.value == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic151 = {
      autorizo: opcion_hic151.value == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      testigo: getTestigo,
      paciente: getPaci,
      prof: getProf,
    ...
    const docDefinitionPrint = await utilsFormat({
      datos: { ...firmas, cod_consen: "HIC151" },
      content: impresionHIC151({
        datos: datos_hic151,
      }),
    });
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-151` });
