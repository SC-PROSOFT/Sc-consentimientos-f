<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic158 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic158 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic158">
              {{ opcion_hic158 }}
            </q-chip>
          </p>
        </div>
        <div class="row q-mt-md q-mb-md">
          <div>
            ...
            <p class="text-justify q-pa-xs">
              La cervicotomía exploradora se realiza para el estudio de una masa cervical de naturaleza desconocida a pesar de los exámenes realizados
              previamente (exámenes bioquímicos, radiológicos, endoscópicos, citológicos...) Esta intervención permite efectuar una exploración
              quirúrgica y extirpar toda o parte de la tumoración para realizar un examen histológico preoperatorio y decidir el tratamiento
              posterior.
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">EN QUE CONSISTE LA CERVICOTOMIA EXPLORADORA</p>
            <p class="text-justify q-pa-xs">
              La cicatriz cervical se realiza alrededor de la masa siguiendo un pliegue cutáneo para disimularla. La intervención consiste en una
              exploración cervical para identificar el origen anatómico de la lesión y el tipo. La masa cervical se extirpará parcial o totalmente
              para permitir la realización de estudios complementarios. Según los resultados la intervención puede ser seguida de otra intervención
              quirúrgica.
            </p>
            <p class="text-justify q-pa-xs">
              Es posible utilizar este abordaje para realizar un vaciamiento ganglionar en el mismo tiempo quirúrgico y/o tiroidectomía. También cabe
              lo posibilidad de que durante la cirugía haya que realizar modificaciones del procedimiento por los hallazgos intraoperatorios para
              proporcionar un tratamiento más adecuado.
            </p>
          </div>
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">RIESGOS DE LA CERVICOTOMIA EXPLORADORA</p>
            <p class="text-justify q-pa-xs">
              A pesar de la adecuado elección de la técnica y de su correcta realización, pueden presentarse efectos indeseables, tanto los comunes
            ...
            <p class="text-justify q-pa-xs">Hemorragia que en casos raros precisará reintervención quirúrgica.</p>
            <p class="text-justify q-pa-xs">Hematoma local.</p>
            <p class="text-justify q-pa-xs">Dehiscencia de la sutura y/o sobreinfección operatoria.</p>
            <p class="text-justify q-pa-xs">Salida de linfa que requiera reintervención o vendajes compresivos durante semanas.</p>
            <p class="text-justify q-pa-xs">
              <span style="text-decoration: underline">Riesgos secundarios:</span>
            </p>
            <p class="text-justify q-pa-xs">
              Alrededor de la cicatrización se puede producir uno fibrosis o estenosamiento de la cicatriz con dolor y zona de anestesia local.
            </p>
            <p class="text-justify q-pa-xs">Dolores cervicales o de espalda con la movilización del brozo.</p>
            <p class="text-justify q-pa-xs">
              <span style="text-decoration: underline">Complicaciones graves:</span>
            </p>
            <p class="text-justify q-pa-xs">Hemorragia grave que requiera reintervención de urgencia.</p>
            <p class="text-justify q-pa-xs">Afectación de nervios locales con dificultad en la alimentación o en la voz.</p>
            <p class="text-justify q-pa-xs">
          ...
          <div class="col-12">
            ...
            <TextArea_ v-model="HIC158.riesgo_personaliz" :field="form.riesgo_personaliz" class="col-12" />
          </div>
          <div>
        ...
        <div v-if="opcion_hic158 == 'AUTORIZAR'">
          ...
          <div>
            <p class="text-justify q-pa-xs">
              ...
              <span class="text-bold">EXPLORADORA.</span>
            </p>
            <p class="text-justify q-pa-xs">
            ...
            <Input_ v-model="HIC158.med_explica" :field="form.med_explica" :inputStyle="{ width: '700px' }" />
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic158 == 'REVOCAR'">
        ...
        <span class="text-bold">Villavicencio, {{ HIC158.fecha }}. </span>
      </q-card-section>
    </div>
    <q-separator />
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic158 ? false : true"
        @click="validarDatos"
...
import { impresionHIC158, impresion, generarArchivo } from "@/impresiones";
...
const HIC158 = reactive({
  fecha: "",
  med_explica: "",
  riesgo_personaliz: "",
});

const opcion_hic158 = ref(null);
...
onMounted(() => {
  HIC158.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC158));
  let datos = {
    nit_entid: nit_usu.value,
    estado: opcion_hic158.value == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic158 = {
      autorizo: opcion_hic158.value == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      testigo: getTestigo,
      paciente: getPaci,
      prof: getProf,
    ...
    const docDefinitionPrint = await utilsFormat({
      datos: { ...firmas, cod_consen: "HIC158" },
      content: impresionHIC158({
        datos: datos_hic158,
      }),
    });
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-158` });
