<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic161 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic161 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic161">
              {{ opcion_hic161 }}
            </q-chip>
          </p>
        </div>
        <div class="row q-mt-md q-mb-md">
          <div>
            ...
            <p class="text-justify q-pa-xs">
              En algunos casos y dependiendo del grosor y del acortamiento del frenillo lengual pueden precisar sutura y/o reconstrucción con
              procedimientos denominados Z plastial.
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">
              EN QUE CONSISTE LA SUTURA DE HERIDA DE CAVIDAD ORAL/OROFARINGE
            </p>
            <p class="text-justify q-pa-xs">
              La intervención se realiza sobre el frenillo localizado debajo de la lengua (cara ventral) . La reseccion del frenillo permite liberar
              la lengua mejorando su mobilidad.
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">
              RIESGOS DE LA SUTURA DE HERIDA DE CAVIDAD ORAL/OROFARINGE
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div class="col-12">
            ...
            <TextArea_ v-model="HIC161.riesgo_personaliz" :field="form.riesgo_personaliz" class="col-12" />
          </div>
          <div>
        ...
        <div v-if="opcion_hic161 == 'AUTORIZAR'">
          ...
          <div>
            <p class="text-justify q-pa-xs">
              ...
              <span class="text-bold">FRENILLECTOMIA.</span>
            </p>
            <p class="text-justify q-pa-xs">
            ...
            <Input_ v-model="HIC161.med_explica" :field="form.med_explica" :inputStyle="{ width: '700px' }" />
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic161 == 'REVOCAR'">
        ...
        <span class="text-bold">Villavicencio, {{ HIC161.fecha }}. </span>
      </q-card-section>
    </div>
    <q-separator />
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic161 ? false : true"
        @click="validarDatos"
...
import { impresionHIC161, impresion, generarArchivo } from "@/impresiones";
...
const HIC161 = reactive({
  fecha: "",
  med_explica: "",
  riesgo_personaliz: "",
});

const opcion_hic161 = ref(null);
...
onMounted(() => {
  HIC161.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC161));
  let datos = {
    nit_entid: nit_usu.value,
    estado: opcion_hic161.value == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic161 = {
      autorizo: opcion_hic161.value == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      testigo: getTestigo,
      paciente: getPaci,
      prof: getProf,
    ...
    const docDefinitionPrint = await utilsFormat({
      datos: { ...firmas, cod_consen: "HIC161" },
      content: impresionHIC161({
        datos: datos_hic161,
      }),
    });
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-161` });
