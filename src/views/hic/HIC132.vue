<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic132 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic132 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic132">
              {{ opcion_hic132 }}
            </q-chip>
          </p>
        </div>
        <div class="row q-mt-md q-mb-md">
          ...
          <div class="col-12">
            ...
            <TextArea_ v-model="HIC132.riesgo_personaliz" :field="form.riesgo_personaliz" class="col-12" />
          </div>
          <div>
            ...
            <p class="text-justify q-pa-xs">
              ...
              existe una alternativa mejor.Si después de leer detenidamente este documento desea más información, por favor, no dude en preguntar al
              especialista responsable, que le atenderá con mucho gusto.
            </p>
          </div>
        </div>
        <div v-if="opcion_hic132 == 'AUTORIZAR'">
          ...
          <div>
            <p class="text-justify q-pa-xs">
              ...
              <span class="text-bold">TURBINOPLASTIA BILATERAL. </span>
            </p>
            <p class="text-justify q-pa-xs">
            ...
            <Input_ v-model="HIC132.med_explica" :field="form.med_explica" :inputStyle="{ width: '700px' }" />
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic132 == 'REVOCAR'">
        ...
        <span class="text-bold">Villavicencio, {{ HIC132.fecha }}. </span>
      </q-card-section>
    </div>
    <q-separator />
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic132 ? false : true"
        @click="validarDatos"
...
import { impresionHIC132, impresion, generarArchivo } from "@/impresiones";
...
const HIC132 = reactive({
  fecha: "",
  med_explica: "",
  riesgo_personaliz: "",
});

const opcion_hic132 = ref(null);
...
onMounted(() => {
  HIC132.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC132));
  let datos = {
    nit_entid: nit_usu.value,
    estado: opcion_hic132.value == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic132 = {
      autorizo: opcion_hic132.value == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      testigo: getTestigo,
      paciente: getPaci,
      prof: getProf,
    ...
    const docDefinitionPrint = await utilsFormat({
      datos: { ...firmas, cod_consen: "HIC132" },
      content: impresionHIC132({
        datos: datos_hic132,
      }),
    });
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-132` });
