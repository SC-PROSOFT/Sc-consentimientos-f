<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic164 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic164 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic164">
              {{ opcion_hic164 }}
            </q-chip>
          </p>
        </div>
        <div class="row q-mt-md q-mb-md">
          <div>
            ...
            <p class="text-justify q-pa-xs">
              Se indica la resección quirúrgica sobre fístulas o quistes preariculares, defectos congénitos que con frecuencia producen salida de
              secreción mal oliente, dolor e infección, de igual manera realizar un estudio histológico completo.
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">
              EN QUE CONSISTE LA EXTIRPACIÓN DE FISTULA Y/O QUISTES PREAURICULARES
            </p>
            <p class="text-justify q-pa-xs">
              La ablación de este tipo de lesión puede ser precedida de una biopsia aunque ésta no se realiza de forma sistemática, ya que el aspecto
              de la lesión, a veces, es suficientemente significativo. Las fístulas y/o quistes preauriculares se operan dejando un margen de
              seguridad de aproximadamente 1 mm de piel sana alreedor de la lesión. La reparación de la pérdida de sustancia realizada al extirpar la
              lesión se puede hacer mediante sutura directa, mediante un colgajo de piel de la proximidad, mediante un injerto total de piel, que se
              puede extraer de detrás de la oreja o de debajo de la clavícula, mediante un colgajo de piel de una región distante (región frontal o
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">
              RIESGOS DE LA EXTIRPACIÓN DE LAS FÍSTULAS Y/O QUISTES PREAURICULARES
            </p>
            <p class="text-justify q-pa-xs">
            ...
            <ul class="text-justify">
              ...
              <li>
                <p class="text-justify q-pa-xs">
                  La sección de una o varias ramas del nervio facial en la proximidad de estas lesiones que puede entrañar la aparición de una
                  asimetría facial habitualmente limitada.
                </p>
              </li>
              <li>
                <p class="text-justify q-pa-xs">
              ...
              <li>
                <p class="text-justify q-pa-xs">Una cicatrización no satisfactoria, puede requerir reintervención.</p>
              </li>
            </ul>
            <p class="text-justify q-pa-xs">
          ...
          <div class="col-12">
            ...
            <TextArea_ v-model="HIC164.riesgo_personaliz" :field="form.riesgo_personaliz" class="col-12" />
          </div>
          <div>
        ...
        <div v-if="opcion_hic164 == 'AUTORIZAR'">
          ...
          <div>
            <p class="text-justify q-pa-xs">
              ...
              <span class="text-bold">EXTIRPACIÓN DE FÍSTULAS Y/ O QUISTES PREAURICULARES.</span>
            </p>
            <p class="text-justify q-pa-xs">
            ...
            <Input_ v-model="HIC164.med_explica" :field="form.med_explica" :inputStyle="{ width: '700px' }" />
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic164 == 'REVOCAR'">
        ...
        <span class="text-bold">Villavicencio, {{ HIC164.fecha }}. </span>
      </q-card-section>
    </div>
    <q-separator />
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic164 ? false : true"
        @click="validarDatos"
...
import { impresionHIC164, impresion, generarArchivo } from "@/impresiones";
...
const HIC164 = reactive({
  fecha: "",
  med_explica: "",
  riesgo_personaliz: "",
});

const opcion_hic164 = ref(null);
...
onMounted(() => {
  HIC164.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC164));
  let datos = {
    nit_entid: nit_usu.value,
    estado: opcion_hic164.value == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic164 = {
      autorizo: opcion_hic164.value == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      testigo: getTestigo,
      paciente: getPaci,
      prof: getProf,
    ...
    const docDefinitionPrint = await utilsFormat({
      datos: { ...firmas, cod_consen: "HIC164" },
      content: impresionHIC164({
        datos: datos_hic164,
      }),
    });
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-164` });
