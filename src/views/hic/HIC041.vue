<template>
  <q-card class="q-mx-auto format">
    <q-form @submit="validarDatos">
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="HIC041.opcion_hc041 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="HIC041.opcion_hc041 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="HIC041.opcion_hc041">
              {{ HIC041.opcion_hc041 }}
            </q-chip>
          </p>
        </div>
        <div class="row">
          ...
          <q-input disable type="text" dense v-model="HIC041.llave" class="col-1" />
        </div>
        <div class="row">
          <p>Ciudad:</p>
        ...
        <div class="row justify-start">
          ...
          <Input_ style="min-width: 100%; display: inline-block" v-model="HIC041.complicaciones" :field="form.complicaciones" />
        </div>
        <p align="justify">
        ...
        <div class="row" v-show="HIC041.opcion_hc041 == 'AUTORIZAR'">
          <p>
            ...
            salud:
            <Input_ style="min-width: 100%; display: inline-block" v-model="HIC041.procedimiento" :field="form.procedimiento" />
            cuyo objetivo es:
            <Input_ style="min-width: 100%; display: inline-block" v-model="HIC041.objetivo" :field="form.objetivo" />
            ante el diagnostico o impresion diagnostica
            <Input_
              style="min-width: 100px; display: inline-block"
              @validate="datoCodigoEnfermedad"
              v-model="HIC041.diagnostico"
              :field="form.codigo"
            />
            <q-input dense disable type="text" maxlength="4" v-model="descrip_diagnostico" style="min-width: 300px; display: inline-block" />
          </p>
        </div>
        <div class="row" v-show="HIC041.opcion_hc041 == 'REVOCAR'">
          <p align="justify">
          ...
          <Input_ style="min-width: 100%; display: inline-block" v-model="HIC041.revocar_motivos" :field="form.revocar_motivos" />
        </div>
      </q-card-section>
    </q-form>
    <q-separator />
    ...
    <div class="row justify-center q-my-lg">
      <q-btn
        :disable="HIC041.opcion_hc041 ? false : true"
        @click="validarDatos"
...
import { impresionHIC041, impresion, generarArchivo } from "@/impresiones";
...
const { getDll$, _getFirma$, guardarFile$, enviarCorreo$, getEncabezado, _getHuella$ } = useApiContabilidad();
...
const datosInit = () => {
  HIC041.fecha_act = dayjs(getEmpresa.FECHA_ACT).format("YYYY-MM-DD");
  HIC041.llave = getHc.llave.slice(15);

  if (getHc.rips.diagn.length) {
    HIC041.diagnostico = getHc.rips.diagn[0].cod;
...
const consultarEnfermedad = async () => {
  try {
    const response = await getDll$({
      modulo: `get_enf.dll`,
      data: { llave: "2" + HIC041.diagnostico },
    });
    if (response.llave) {
...
const validarDatos = async () => {
  ...
  if (!HIC041.complicaciones) return CON851("?", "info", `${requiere}, complicaciones `, () => foco_(form, "complicaciones"));

  if (HIC041.opcion_hc041 == "REVOCAR") {
    if (!HIC041.revocar_motivos) return CON851("?", "info", `${requiere}, revocar motivos `, () => foco_(form, "revocar_motivos"));
  }

  if (HIC041.opcion_hc041 == "AUTORIZAR") {
    if (!HIC041.diagnostico) return CON851("?", "info", requiere, () => foco_(form, "codigo"));
    if (!HIC041.procedimiento) return CON851("?", "info", requiere, () => foco_(form, "procedimiento"));
    if (!HIC041.objetivo) return CON851("?", "info", requiere, () => foco_(form, "objetivo"));
  }
  grabarConsentimiento();
};

const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC041));
  let datos = {
    estado: HIC041.opcion_hc041 == "AUTORIZAR" ? "1" : "2",
  ...
  await getDll$({ modulo: `save_consen.dll`, data: { ...datos } })
    .then((data) => {
      if (data?.llave_consen) {
        ...
        HIC041.fecha_act = dayjs(fecha).format("YYYY-MM-DD");
...
const imprimirConsen = async (llave) => {
  const datos_hic041 = {
    autorizo: HIC041.opcion_hc041 == "AUTORIZAR" ? true : false,
    empresa: getEmpresa,
    paciente: getPaci,
    prof: getProf,
    acomp: getAcomp,
  ...
  const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-041` });
  return response_impresion;
};
const callbackCONSEN800 = (data) => {
  if (data) {
    HIC041.diagnostico = data.cod;
