<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic149 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic149 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic149">
              {{ opcion_hic149 }}
            </q-chip>
          </p>
        </div>
        <div class="row q-mt-md q-mb-md">
          <div>
            ...
            <p class="text-justify q-pa-xs">
              La uvulopalatofaringoplastia con/sin amigdalectomía es una resección de las estructuras de la faringe que pretende dar tensión al velo
              del paladar y aumentar el calibre de la faringe. Esta cirugía se realiza habitualmente en caso de ronquidos y/o síndrome de apnea del
              sueño obstructivo.
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">EN QUE CONSISTE LA UVULOPALATOFARINGOPLASTIA</p>
            <p class="text-justify q-pa-xs">
              La intervención se realiza por vía bucal sin cicatriz exterior. Se realiza una resección de parte del paladar blando con/sin extirpación
              de la úvula si es preciso. Suele asociarse a una amigdalectomía.
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">RIESGOS DE LA UVULOPALATOFARINGOPLASTIA</p>
            <p class="text-justify q-pa-xs">
            ...
            <p class="text-justify q-pa-xs">
              ...
              Lesiones en boca, lengua y/o dientes por la utilización de instrumentos de apertura bucal; escaras hemorrágicas en la zona de cirugía;
              dolor con la deglución que cede con analgesia; de forma transitoria puede aparecer regurgitación de líquidos por la nariz;
              sobreinfección local.
            </p>
            <p class="text-justify q-pa-xs">
              <span style="text-decoration: underline">Riesgos secundarios:</span>
              Modificación de la voz por fuga de aire; persistencia de regurgitación de líquidos por la nariz; parestesias faríngeas; control parcial
              de los síntomas a pesar de una técnica quirúrgica correcta.
            </p>
            <p class="text-justify q-pa-xs">
              <span style="text-decoration: underline">Complicaciones graves excepcionales:</span>
              Estenosis faríngea a nivel del velo del paladar; incontinencia del velo del paladar. Estas complicaciones habitualmente se resuelven con
              tratamiento médico (medicamentos, sueros, etc.) pero pueden llegar a requerir una reintervención, en algunos casos de urgencia.
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div class="col-12">
            ...
            <TextArea_ v-model="HIC149.riesgo_personaliz" :field="form.riesgo_personaliz" class="col-12" />
          </div>
          <div>
        ...
        <div v-if="opcion_hic149 == 'AUTORIZAR'">
          ...
          <div>
            <p class="text-justify q-pa-xs">
              ...
              <span class="text-bold">UVULOPALATOFARINGOPLASTIA.</span>
            </p>
            <p class="text-justify q-pa-xs">
            ...
            <Input_ v-model="HIC149.med_explica" :field="form.med_explica" :inputStyle="{ width: '700px' }" />
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic149 == 'REVOCAR'">
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic149 ? false : true"
        @click="validarDatos"
...
import { impresionHIC149, impresion, generarArchivo } from "@/impresiones";
...
const HIC149 = reactive({
  fecha: "",
  med_explica: "",
  riesgo_personaliz: "",
});

const opcion_hic149 = ref(null);
...
onMounted(() => {
  HIC149.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC149));
  let datos = {
    nit_entid: nit_usu.value,
    estado: opcion_hic149.value == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic149 = {
      autorizo: opcion_hic149.value == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      testigo: getTestigo,
      paciente: getPaci,
      prof: getProf,
    ...
    const docDefinitionPrint = await utilsFormat({
      datos: { ...firmas, cod_consen: "HIC149" },
      content: impresionHIC149({
        datos: datos_hic149,
      }),
    });
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-149` });
