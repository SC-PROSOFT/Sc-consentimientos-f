<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic080 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic080 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic080">
              {{ opcion_hic080 }}
            </q-chip>
          </p>
        </div>
        ...
        <div v-if="opcion_hic080 == 'REVOCAR'">
          ...
          <p style="text-align: justify">
            ...
            libre y voluntariamente REVOCO el consentimiento prestado con fecha <span class="text-bold">{{ HIC080.fecha }}</span> ante el Higienista
            Dental y/o Dra. Informante:
          </p>
          <Input_ v-model="HIC080.profes_infor_revoc" :field="form.profes_infor_revoc" />
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic080 ? false : true"
        @click="validarDatos"
...
const opcion_hic080 = ref(null);
const autoriza = computed(() => {
  switch (opcion_hic080.value) {
    case "AUTORIZAR":
      return "S";
    case "REVOCAR":
      return "N";
...
onMounted(() => {
  HIC080.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC080));
  let datos = {
...
const imprimirConsen = async (llave) => {
  try {
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-080` });
