<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic126 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic126 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic126">
              {{ opcion_hic126 }}
            </q-chip>
          </p>
        </div>
        ...
        <div class="row q-mt-md q-mb-md">
          <div>
            ...
            <p class="text-justify q-pa-xs">
              La extirpación de un ganglio, adenectomía, se realiza para determinar la naturaleza de una adenopatía de origen cervical. La aparición
              de dicha adenopatía puede ser de origen infeccioso, puede ser debido a una infección o de origen tumoral, benigno o maligno. En función
              de la localización de la adenopatía, la anestesia requerida para realizar la intervención puede ser local o general.
            </p>
            <p class="text-justify q-pa-xs">
              El tipo de anestesia requerida será la indicada por el anestesiólogo. & posible que, durante o después de la intervención, sea necesaria
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">EN QUE CONSISTE LA ADENECTOMÍA CERVICAL</p>
            <p class="text-justify q-pa-xs">
              La intervención se efectúa a través de una pequeña incisión alrededor del ganglio. B ganglio puede extirparse completamente o realizarse
              una biopsia en cuña para llevar a cabo diferentes análisis (bacteriológico, anotomopatologico...) Puede ser requerido en algunas
              ocasiones un examen histológico peroperatorio para precisar la naturaleza de la lesión y determinar el acto quirúrgico posterior.
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">RIESGOS DE LA ADENECTOMÍA CERVICAL</p>
            <p class="text-justify q-pa-xs">
            ...
            <ul class="text-justify">
              <li>
                <p class="text-justify q-pa-xs">
                  ...
                  Hematoma local, Supuración local que puede conllevar una alteración en la cicatrización.
                </p>
              </li>
              <li>
                <p class="text-justify q-pa-xs">
                  <span style="text-decoration: underline">Riesgos secundarios:</span>
                  Hipertrofia de la cicatriz con dolor local y pequeña zona de anestesia a nivel de la misma. Dolores cervicales o de espalda
                  pasajeros.
                </p>
                <p class="text-justify q-pa-xs">
                  Dependiendo de los resultados histológicos definitivos es posible la reintervención para ampliar cirugía local.
                </p>
              </li>
              <li>
                <p class="text-justify q-pa-xs">
                  <span style="text-decoration: underline">Complicaciones graves:</span>
                  En algunas ocasiones es preciso la extirpación de un nervio laríngeo que puede producir problemas en la voz o dificultades en la
                  alimentación
                </p>
              </li>
            </ul>
          ...
          <div class="col-12">
            ...
            <TextArea_ v-model="HIC126.riesgo_personaliz" :field="form.riesgo_personaliz" class="col-12" />
          </div>
          <div>
        ...
        <div v-if="opcion_hic126 == 'AUTORIZAR'">
          ...
          <div>
            <p class="text-justify q-pa-xs">
              ...
              <span class="text-bold">ADENECTOMIA CERVICAL. </span>
            </p>
            <p class="text-justify q-pa-xs">
            ...
            <Input_ v-model="HIC126.med_explica" :field="form.med_explica" :inputStyle="{ width: '700px' }" />
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic126 == 'REVOCAR'">
        ...
        <span class="text-bold">Villavicencio, {{ HIC126.fecha }}. </span>
      </q-card-section>
    </div>
    <q-separator />
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic126 ? false : true"
        @click="validarDatos"
...
import { impresionHIC126, impresion, generarArchivo } from "@/impresiones";
...
const HIC126 = reactive({
  fecha: "",
  med_explica: "",
  riesgo_personaliz: "",
});

const opcion_hic126 = ref(null);
...
onMounted(() => {
  HIC126.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC126));
  let datos = {
    nit_entid: nit_usu.value,
    estado: opcion_hic126.value == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic126 = {
      autorizo: opcion_hic126.value == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      testigo: getTestigo,
      paciente: getPaci,
      prof: getProf,
    ...
    const docDefinitionPrint = await utilsFormat({
      datos: { ...firmas, cod_consen: "HIC126" },
      content: impresionHIC126({
        datos: datos_hic126,
      }),
    });
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-126` });
