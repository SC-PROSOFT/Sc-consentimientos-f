<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic148 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic148 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic148">
              {{ opcion_hic148 }}
            </q-chip>
          </p>
        </div>
        <div class="row q-mt-md q-mb-md">
          <div>
            ...
            <p class="text-justify q-pa-xs">
              El angiofibroma de rinofaringe es un tumor benigno que crece en la porción posterior de los fosas nasales y que está integrado, en buena
              medida, por vasos sanguíneos, por lo que se trata de una tumoradón extraordinariamente sangrante. Por ello, la intervención quirúrgica
              puede requerir la embolización del tumor unos días antes para conseguir la obstrucción de los vasos sanguíneos que llegan al tumor. El
              angiofíbroma se reproduce en un porcentaje elevado de los casos, por lo que es necesaria la revisión periódica de la cavidad operatoria.
            </p>
            <p class="text-justify q-pa-xs">
              El tipo de anestesia requerida será lo indicada por el anestesiólogo. Es posible que, durante o después de la intervención, sea
              ...
              alteraciones de la coagulación, enfermedades cordiopulmonares, existencia de prótesis, marcapasos, medicaciones actuales o cualquier
              otra circunstancia.
            </p>
          </div>
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">
              EN QUE CONSISTE LA EXTIRPACIÓN DEL ANGIOFIBROMA DE RINOFARINGE
            </p>
            <p class="text-justify q-pa-xs">
              La vía de abordaje es variable en función del tamaño y la localización del tumor pudiendo ser externa ( rinotomía lateral ), via oral (
              transpalatina), endoscópica ( via nasal ) o combinada . La incisión es una rinotomía lateronasal, que se realiza en la cara, por dentro
              del ojo, al lado de la nariz y siguiendo el eje de la pirámide nasal. En ocasiones, la incisión puede prolongarse a lo largo del labio
              superior y la encía.
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">
              RIESGOS DE LA EXTIRPACIÓN DEL ANGIOFIBROMA DE RINOFARINGE
            </p>
            <p class="text-justify q-pa-xs">
            ...
            <ul class="text-justify">
              <li>
                <p class="text-justify q-pa-xs">
                  Hemorragia, que se previene con el taponamiento nasal, pero que puede aparecer a pesar del mismo precisando la revisión de la
                  cavidad operatoria y/o el cambio del taponamiento.
                </p>
              </li>
              <li>
                <p class="text-justify q-pa-xs">
                  Infección de la cavidad operatoria, de los senos o la cavidad craneal (sinusitis, meningitis, encefalitis, abscesos cerebrales,
                  etc.).
                </p>
              </li>
              <li>
                <p class="text-justify q-pa-xs">Fistulización -fuga del líquido cefalorraquídeo- hacia el interior de la fosa nasal o la faringe.</p>
              </li>
              <li>
                <p class="text-justify q-pa-xs">Cefaleas de intensidad y localización variables.</p>
              </li>
              <li>
                <p class="text-justify q-pa-xs">
                  Complicaciones oculares: visión doble, infección de la órbita, e incluso ceguera por afectación del nervio ocular.
                </p>
              </li>
              <li>
                <p class="text-justify q-pa-xs">
                  Perforaciones del tabique nasal, sinequias -bridas entre las paredes de la fosa nasal-, trastornos del olfato, sequedad y formación
                  de costras de moco.
                </p>
              </li>
              <li>
                <p class="text-justify q-pa-xs">La cicatriz puede ser inestética y, en ocasiones, doloroso.</p>
              </li>
            </ul>
            <p class="text-justify q-pa-xs">
          ...
          <div class="col-12">
            ...
            <TextArea_ v-model="HIC148.riesgo_personaliz" :field="form.riesgo_personaliz" class="col-12" />
          </div>
          <div>
        ...
        <div v-if="opcion_hic148 == 'AUTORIZAR'">
          ...
          <div>
            <p class="text-justify q-pa-xs">
              ...
              <span class="text-bold">EXTIRPACIÓN DEL ANGIOFIBROMA DE RINOFARINGE POR VIA ENDOSCOPICA.</span>
            </p>
            <p class="text-justify q-pa-xs">
            ...
            <Input_ v-model="HIC148.med_explica" :field="form.med_explica" :inputStyle="{ width: '700px' }" />
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic148 == 'REVOCAR'">
        ...
        <span class="text-bold">Villavicencio, {{ HIC148.fecha }}. </span>
      </q-card-section>
    </div>
    <q-separator />
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic148 ? false : true"
        @click="validarDatos"
...
import { impresionHIC148, impresion, generarArchivo } from "@/impresiones";
...
const HIC148 = reactive({
  fecha: "",
  med_explica: "",
  riesgo_personaliz: "",
});

const opcion_hic148 = ref(null);
...
onMounted(() => {
  HIC148.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC148));
  let datos = {
    nit_entid: nit_usu.value,
    estado: opcion_hic148.value == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic148 = {
      autorizo: opcion_hic148.value == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      testigo: getTestigo,
      paciente: getPaci,
      prof: getProf,
    ...
    const docDefinitionPrint = await utilsFormat({
      datos: { ...firmas, cod_consen: "HIC148" },
      content: impresionHIC148({
        datos: datos_hic148,
      }),
    });
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-148` });
