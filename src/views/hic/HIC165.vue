<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic165 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic165 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic165">
              {{ opcion_hic165 }}
            </q-chip>
          </p>
        </div>
        <div class="row q-mt-md q-mb-md">
          <div>
            ...
            <p class="text-justify q-pa-xs">
              Mediante este procedimiento se pretende la toma de muestra de glandula salivar menor para realizar un estudio anatomopatológico de la
              misma. El tipo de anestesia requerida será la indicada por el anestesiólogo, aunque en principio, se realizará bajo anestesia local. Es
              poco frecuente pero posible que, durante o después de la intervención, sea necesaria la utilización de sangre y/o hemoderivados. También
              es necesario que advierta de sus posibles alergias medicamentosas, alteraciones de la coagulación, enfermedades cardiopulmonares,
              existencia de prótesis, marcapasos, medicaciones actuales o cualquier otra circunstancia.
            </p>
          </div>
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">EN QUE CONSISTE LA BIOPSIA BAJO ANESTESIA LOCAL</p>
            <p class="text-justify q-pa-xs">
              Tras la infiltración con anestesia local en la mucosa labial , se extirpa una zona de la mucosa junto al tejido glandular circundante.
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">RIESGOS DE LA BIOPSIA BAJO ANESTESIA LOCAL</p>
            <p class="text-justify q-pa-xs">
            ...
            <p class="text-justify q-pa-xs">
              ...
              o antiestéticas, molestias locales, dolor prolongado en la zona de la operación areas de hipoestesia ( sensación de adormecimiento)
              transitorias y rara vez permanentes.
            </p>
            <p class="text-justify q-pa-xs">
              <span style="text-decoration: underline">Poco frecuentes y graves:</span>
              Reacción alérgica grave a los anestésicos locales que puede llegar a producir shock anafiláctico.
            </p>

            <p class="text-justify q-pa-xs">
              Estas complicaciones, habitualmente, se resuelven con tratamiento médico y, a veces, pueden llegar a requerir una reintervención, en
              algunos casos de urgencia (evacuación de un hematoma, drenaje de un absceso, etc.).
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div class="col-12">
            ...
            <TextArea_ v-model="HIC165.riesgo_personaliz" :field="form.riesgo_personaliz" class="col-12" />
          </div>
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">QUE OTRAS ALTERNATIVAS HAY</p>
            <p class="text-justify q-pa-xs">En su caso no existe una alternativa terapéutica eficaz.</p>
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic165 == 'AUTORIZAR'">
          ...
          <div class="row">
            <p class="text-justify">
              ...
              <span class="text-bold">EXTIRPACIÓN DE GLANDULA SALIVAR MENOR BAJO ANESTESIA LOCAL.</span>
            ...
            <Input_ v-model="HIC165.med_explica" :field="form.med_explica" :inputStyle="{ width: '700px' }" />
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic165 == 'REVOCAR'">
        ...
        <span class="text-bold">Villavicencio, {{ HIC165.fecha }}. </span>
      </q-card-section>
    </div>
    <q-separator />
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic165 ? false : true"
        @click="validarDatos"
...
import { impresionHIC165, impresion, generarArchivo } from "@/impresiones";
...
const HIC165 = reactive({
  fecha: "",
  med_explica: "",
  riesgo_personaliz: "",
});

const opcion_hic165 = ref(null);
...
onMounted(() => {
  HIC165.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC165));
  let datos = {
    nit_entid: nit_usu.value,
    estado: opcion_hic165.value == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic165 = {
      autorizo: opcion_hic165.value == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      testigo: getTestigo,
      paciente: getPaci,
      prof: getProf,
    ...
    const docDefinitionPrint = await utilsFormat({
      datos: { ...firmas, cod_consen: "HIC165" },
      content: impresionHIC165({
        datos: datos_hic165,
      }),
    });
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-165` });
