<template>
  <q-card class="q-mx-auto format">
    <q-form @submit="validarDatos">
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="HIC037.opcion_hc037 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="HIC037.opcion_hc037 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="HIC037.opcion_hc037">
              {{ HIC037.opcion_hc037 }}
            </q-chip>
          </p>
        </div>
        <div class="row">
          ...
          <q-input disable type="text" dense v-model="HIC037.llave" class="col-1" />
        </div>
        <div class="row">
          <p>Ciudad:</p>
          <q-input disable type="text" dense class="col-1" v-model="getEmpresa.ciudad_usuar" />
          <p>fecha:</p>
          <q-input disable type="text" dense class="col-1" v-model="HIC037.fecha_act" />
        </div>
        ...
        <div class="row justify-start">
          ...
          <Input_ style="min-width: 100%; display: inline-block" v-model="HIC037.complicaciones" :field="form.complicaciones" />
        </div>
        <p align="justify">
        ...
        <div class="row" v-show="HIC037.opcion_hc037 == 'AUTORIZAR'">
          <p>
            ...
            salud:
            <Input_ style="min-width: 100%; display: inline-block" v-model="HIC037.procedimiento" :field="form.procedimiento" />
            cuyo objetivo es:
            <Input_ style="min-width: 100%; display: inline-block" v-model="HIC037.objetivo" :field="form.objetivo" />
            diagnostico
            <Input_
              style="min-width: 100px; display: inline-block"
              @validate="datoCodigoEnfermedad"
              v-model="HIC037.diagnostico"
              :field="form.codigo"
            />
            <q-input dense disable type="text" maxlength="4" v-model="descrip_diagnostico" style="min-width: 300px; display: inline-block" />
          </p>
        </div>
        <div class="row" v-show="HIC037.opcion_hc037 == 'REVOCAR'">
          <p align="justify">
          ...
          <Input_ style="min-width: 100%; display: inline-block" v-model="HIC037.revocar_motivos" :field="form.revocar_motivos" />
        </div>
      </q-card-section>
    </q-form>
    <q-separator />
    ...
    <div class="row justify-center q-my-lg">
      <q-btn
        :disable="HIC037.opcion_hc037 ? false : true"
        @click="validarDatos"
...
import { impresionHIC037, impresion, generarArchivo } from "@/impresiones";
...
const datosInit = () => {
  HIC037.fecha_act = dayjs(getEmpresa.FECHA_ACT).format("YYYY-MM-DD");
  HIC037.llave = getHc.llave.slice(15);

  if (getHc.rips.diagn.length) {
    HIC037.diagnostico = getHc.rips.diagn[0].cod;
...
const consultarEnfermedad = async () => {
  try {
    const response = await getDll$({
      modulo: `get_enf.dll`,
      data: { llave: "2" + HIC037.diagnostico },
    });
    if (response.llave) {
...
const validarDatos = async () => {
  ...
  if (!HIC037.complicaciones) return CON851("?", "info", `${requiere}, complicaciones `, () => foco_(form, "complicaciones"));

  if (HIC037.opcion_hc037 == "REVOCAR") {
    if (!HIC037.revocar_motivos) return CON851("?", "info", `${requiere}, revocar motivos `, () => foco_(form, "revocar_motivos"));
  }

  if (HIC037.opcion_hc037 == "AUTORIZAR") {
    if (!HIC037.diagnostico) return CON851("?", "info", requiere, () => foco_(form, "codigo"));
    if (!HIC037.procedimiento) return CON851("?", "info", requiere, () => foco_(form, "procedimiento"));
    if (!HIC037.objetivo) return CON851("?", "info", requiere, () => foco_(form, "objetivo"));
  }
  grabarConsentimiento();
};

const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC037));
  let datos = {
    estado: HIC037.opcion_hc037 == "AUTORIZAR" ? "1" : "2",
  ...
  await getDll$({ modulo: `save_consen.dll`, data: { ...datos } })
    .then((data) => {
      if (data?.llave_consen) {
        const fecha = data?.llave_consen.slice(24, 37);
        HIC037.fecha_act = dayjs(fecha).format("YYYY-MM-DD");
...
const imprimirConsen = async (llave) => {
  const datos_HIC037 = {
    autorizo: HIC037.opcion_hc037 == "AUTORIZAR" ? true : false,
    empresa: { ...getEmpresa },
    paciente: { ...getPaci },
    prof: { ...getProf },
    acomp: { ...getAcomp },
  ...
  const response_impresion = await generarArchivo({
    ...
    nomb_archivo: `${llave}-HIC-037`,
  });
  return response_impresion;
};
const callbackCONSEN800 = (data) => {
  if (data) {
    HIC037.diagnostico = data.cod;
