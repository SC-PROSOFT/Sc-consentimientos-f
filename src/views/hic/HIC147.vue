<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic147 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic147 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic147">
              {{ opcion_hic147 }}
            </q-chip>
          </p>
        </div>
        <div class="row q-mt-md q-mb-md">
          ...
          <div>
            <p class="text-left" style="margin-top: 10px; font-weight: bold; margin-left: 10px">RIESGOS DE LA CIRUGÍA</p>
            <p class="text-justify q-pa-xs">
            ...
            <ul class="text-justify">
              ...
              <li>
                <p class="text-justify q-pa-xs">
                  Infecciones tanto a nivel del oído como a nivel cerebral, tales como meningitis o abscesos cerebrales.
                </p>
              </li>
              <li>
          ...
          <div class="col-12">
            ...
            <TextArea_ v-model="HIC147.riesgo_personaliz" :field="form.riesgo_personaliz" class="col-12" />
          </div>
          <div>
        ...
        <div v-if="opcion_hic147 == 'AUTORIZAR'">
          ...
          <div>
            ...
            <Input_ v-model="HIC147.med_explica" :field="form.med_explica" :inputStyle="{ width: '700px' }" />
            <p class="text-justify q-pa-xs">
        ...
        <div v-if="opcion_hic147 == 'REVOCAR'">
        ...
        <span class="text-bold">Villavicencio, {{ HIC147.fecha }}. </span>
      </q-card-section>
    </div>
    <q-separator />
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic147 ? false : true"
        @click="validarDatos"
...
import { impresionHIC147, impresion, generarArchivo } from "@/impresiones";
...
const HIC147 = reactive({
  fecha: "",
  med_explica: "",
  riesgo_personaliz: "",
});

const opcion_hic147 = ref(null);
...
onMounted(() => {
  HIC147.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC147));
  let datos = {
    nit_entid: nit_usu.value,
    estado: opcion_hic147.value == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic147 = {
      autorizo: opcion_hic147.value == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      testigo: getTestigo,
      paciente: getPaci,
      prof: getProf,
    ...
    const docDefinitionPrint = await utilsFormat({
      datos: { ...firmas, cod_consen: "HIC147" },
      content: impresionHIC147({
        datos: datos_hic147,
      }),
    });
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-147` });
