<template>
  <q-card class="q-mx-auto format">
    <div>
      <q-card-section>
        <div class="text-center">
          ...
          />
          <p :class="opcion_hic155 == 'AUTORIZAR' ? 'text-green' : 'text-red'">
            <q-chip :color="opcion_hic155 == 'AUTORIZAR' ? 'green' : 'red'" class="text-white" v-if="opcion_hic155">
              {{ opcion_hic155 }}
            </q-chip>
          </p>
        </div>
        <div class="row q-mt-md q-mb-md">
          ...
          <div>
            ...
            <p class="text-justify q-pa-xs">
              ...
              (peso, edad, diabetes, cardiopatía.), y los específicos el procedimiento:
            </p>
            <p class="text-justify q-pa-xs">
          ...
          <div class="col-12">
            ...
            <TextArea_ v-model="HIC155.riesgo_personaliz" :field="form.riesgo_personaliz" class="col-12" />
          </div>
          <div>
        ...
        <div v-if="opcion_hic155 == 'REVOCAR'">
        ...
        <span class="text-bold">Villavicencio, {{ HIC155.fecha }}. </span>
      </q-card-section>
    </div>
    <q-separator />
    ...
    <div class="col-12 row justify-center q-my-md">
      <q-btn
        :disable="opcion_hic155 ? false : true"
        @click="validarDatos"
...
import { impresionHIC155, impresion, generarArchivo } from "@/impresiones";
...
onMounted(() => {
  HIC155.fecha = dayjs(getEmpresa.fecha_act).format("YYYY-MM-DD");
  getFirmaProf();
});
...
const grabarConsentimiento = async () => {
  const datos_format = JSON.parse(JSON.stringify(HIC155));
  let datos = {
    nit_entid: nit_usu.value,
    estado: opcion_hic155.value == "AUTORIZAR" ? "1" : "2",
...
const imprimirConsen = async (llave) => {
  try {
    const datos_hic155 = {
      autorizo: opcion_hic155.value == "AUTORIZAR" ? true : false,
      empresa: getEmpresa,
      testigo: getTestigo,
      paciente: getPaci,
      prof: getProf,
    ...
    const docDefinitionPrint = await utilsFormat({
      datos: { ...firmas, cod_consen: "HIC155" },
      content: impresionHIC155({
        datos: datos_hic155,
      }),
    });
    ...
    const response_impresion = await generarArchivo({ docDefinition: docDefinitionFile, nomb_archivo: `${llave}-HIC-155` });
